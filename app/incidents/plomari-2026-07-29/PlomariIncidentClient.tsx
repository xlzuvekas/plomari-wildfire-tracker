...
// This is the latest evidence embedded in this archived incident view. It is
// not an official end time, containment statement, or all-clear.
const INCIDENT_ARCHIVE_AS_OF_AT = FIELD_REPORT_OCCURRED_AT;
const INCIDENT_ARCHIVE_AS_OF_EPOCH = Date.parse(INCIDENT_ARCHIVE_AS_OF_AT);
...
export default function PlomariIncidentClient() {
  ...
  const pendingThermalAsOfEpoch = useRef<number | null>(
    INCIDENT_ARCHIVE_AS_OF_EPOCH,
  );
  ...
  const [asOfEpoch, setAsOfEpoch] = useState<number | null>(
    INCIDENT_ARCHIVE_AS_OF_EPOCH,
  );
  ...
  const incidentArchiveAsOfLabel = formatAreaDateTime(
    INCIDENT_ARCHIVE_AS_OF_AT,
    language,
  );
  ...
  const satellitePassLayerDetail = satellitePassPresentation ===
    ...
    ? localize(
        language,
        "Archive view · current CMR catalog coverage not polled",
        "Προβολή αρχείου · δεν γίνεται έλεγχος τρέχουσας κάλυψης CMR",
      )
  ...
  }, [isLive, recordSnapshotResponses]);
  ...
  return (
    ...
    >
        ...
        <div className="brand-lockup">
          <div className="brand-line">
            <span
              ...
              aria-hidden="true"
            />
            <h1>FIREWATCH // PLOMARI ARCHIVE</h1>
          </div>
          <p>
            {localize(
              language,
              "HISTORICAL INCIDENT RECORD · MULTISOURCE OSINT",
              "ΙΣΤΟΡΙΚΟ ΑΡΧΕΙΟ ΣΥΜΒΑΝΤΟΣ · ΠΛΗΡΟΦΟΡΙΕΣ ΑΝΟΙΧΤΩΝ ΠΗΓΩΝ",
            )}
          </p>
        </div>
      ...
      <section
        ...
        aria-label={localize(
          language,
          "Historical incident time",
          "Ιστορικός χρόνος συμβάντος",
        )}
      >
        ...
        <label className="time-scrubber__range">
          ...
          <span className="time-scrubber__bounds">
            ...
            <small>
              {localize(
                language,
                "LATEST INCLUDED EVIDENCE",
                "ΤΕΛΕΥΤΑΙΑ ΣΥΜΠΕΡΙΛΗΦΘΕΙΣΑ ΕΝΔΕΙΞΗ",
              )}{" "}
              ·{" "}
              <time dateTime={INCIDENT_ARCHIVE_AS_OF_AT}>
                {incidentArchiveAsOfLabel}
              </time>
            </small>
          </span>
        </label>
        <button
          ...
          className="is-active"
          onClick={() => {
            pendingThermalAsOfEpoch.current = INCIDENT_ARCHIVE_AS_OF_EPOCH;
            setAsOfEpoch(INCIDENT_ARCHIVE_AS_OF_EPOCH);
            setCommittedThermalAsOfEpoch(INCIDENT_ARCHIVE_AS_OF_EPOCH);
          }}
          aria-pressed={asOfEpoch === INCIDENT_ARCHIVE_AS_OF_EPOCH}
          aria-label={localize(
            language,
            `Return to latest included evidence, ${incidentArchiveAsOfLabel}`,
            `Επιστροφή στην τελευταία συμπεριληφθείσα ένδειξη, ${incidentArchiveAsOfLabel}`,
          )}
        >
          {localize(
            language,
              ...
              },
              {
                ...
                detail: localize(
                  ...
                  isLive
                    ...
                    : "Archive view · current daily composite withheld",
                  isLive
                    ? `ΗΜΕΡΟΜΗΝΙΑ ΖΗΤΟΥΜΕΝΗΣ ΕΙΚΟΝΑΣ · ${requestedImageDate} · ΑΓΝΩΣΤΗ ΩΡΑ ΛΗΨΗΣ`
                    : "Προβολή αρχείου · απόκρυψη τρέχοντος ημερήσιου σύνθετου",
                ),
                count: isLive ? "IMG" : "ARCH",
              },
              {
              ...
              },
              {
                ...
                detail: localize(
                  ...
                  isLive
                    ...
                    : "Archive view · current model not polled",
                  isLive
                    ? fireWind
                    ...
                    : "Προβολή αρχείου · δεν γίνεται έλεγχος τρέχοντος μοντέλου",
                ),
                  ...
                  {localize(
                    ...
                    isLive
                      ? windError
                        ? "MODEL UNAVAILABLE"
                        : "CHECKING MODEL"
                      : "ARCHIVE WIND · NOT POLLED",
                    isLive
                      ? windError
                        ? "ΤΟ ΜΟΝΤΕΛΟ ΔΕΝ ΕΙΝΑΙ ΔΙΑΘΕΣΙΜΟ"
                        : "ΕΛΕΓΧΟΣ ΜΟΝΤΕΛΟΥ"
                      : "ΑΡΧΕΙΟ ΑΝΕΜΟΥ · ΧΩΡΙΣ ΤΡΕΧΟΝΤΑ ΕΛΕΓΧΟ",
                  )}
                </strong>
                <p>
                  {localize(
                    ...
                    isLive
                      ...
                      : "The archive does not poll the latest model or airport observation because those sources cannot reconstruct historical wind.",
                    isLive
                      ? "Δεν εμφανίζονται εφεδρικές τιμές ανέμου. Ο χάρτης θα προσθέσει διανύσματα όταν υπάρχει χρονολογημένη απόκριση μοντέλου."
                      : "Το αρχείο δεν ελέγχει το νεότερο μοντέλο ή την παρατήρηση αεροδρομίου, επειδή αυτές οι πηγές δεν ανασυνθέτουν ιστορικό άνεμο.",
                  )}
                </p>
              </div>
            )}
            ...
            <strong>
              {isLive
                ? updatesData
                  ...
                  : localize(
                      language,
                      "CHECKING SHARED FEED SNAPSHOT",
                      "ΕΛΕΓΧΟΣ ΚΟΙΝΟΥ ΣΤΙΓΜΙΟΤΥΠΟΥ ΡΟΩΝ",
                    )
                : localize(
                    language,
                    "ARCHIVE SNAPSHOT · LIVE FEEDS NOT POLLED",
                    "ΣΤΙΓΜΙΟΤΥΠΟ ΑΡΧΕΙΟΥ · ΧΩΡΙΣ ΕΛΕΓΧΟ ΖΩΝΤΑΝΩΝ ΡΟΩΝ",
                  )}
            </strong>
            <small>
              {localize(
                ...
                isLive
                  ...
                  : "The dated entries below are embedded archive evidence. This view does not call live feeds or publishers.",
                isLive
                  ...
                  : "Οι χρονολογημένες καταχωρίσεις παρακάτω είναι ενσωματωμένα στοιχεία αρχείου. Η προβολή δεν καλεί ζωντανές ροές ή εκδότες.",
              )}
            </small>
          </div>
