...
function dateKeyFromSource(value: string) {
  ...
  // All current ambiguous numeric dates come from Greek sources and are
  // interpreted as DD/MM/YYYY. A future MM/DD/YYYY source needs its own parser.
...
export async function GET(request: Request) {
  ...
  const sources: SourceSnapshot[] = feedResults.map((result, index) => {
    ...
    if (feed === undefined) {
      throw new Error("Internal error: feed result/config mismatch");
    }
    return {
      id: feed.id,
      label: feed.label,
  ...
  if (xResults) {
    xResults.forEach((result, index) => {
      ...
      if (account === undefined) {
        throw new Error("Internal error: X result/account mismatch");
      }
      sources.push({
        id: account.id,
        label: account.label,
